...
class MediaView extends View {
	 ...
	 * @access public
	 */
	public $mimeType = array('ai' => 'application/postscript', 'bcpio' => 'application/x-bcpio', 'bin' => 'application/octet-stream',
								...
								/** core-hack: own ones - 2009-05-11 ms **/
								/*
	...
	public function render() {
		/** content hack (for displaying files on the fly) **/
		$content = $name = $download = $extension = $id = $modified = $path = $size = $cache = $mimeType = null;
		...
		if (!file_exists($path) && empty($content)) {
		...
		/** extension download HACK!!! **/
		if (!isset($this->mimeType[$extension])) {
			$download = true;
		}

		if (isset($extension) && connection_status() == 0) {
			$chunkSize = 8192;
			$buffer = '';
			...
			if ($download) {
				...
				$this->_header(array(
					//'Content-Disposition: filename="' . $name . '.' . $extension . '";', /** hack: does not work **/
			...
			while (!feof($handle) && connection_status() == 0 && !connection_aborted()) {
				...
				try {
					...
					$this->log('Exeption ' . $e->getMessage() . ' in ' . $this->request->url);
				}
			}
			fclose($handle);
			exit(0);
